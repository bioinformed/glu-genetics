...
class GroupKey(object):
  ...
  def __call__(self, row):
    # a header row only has one element (the header),
    # however,illumina inlcudes additional empty elements, thus len(row) >= 1 and not len(row) == 1
...
class SectionWriter(object):
  ...
  same section.
  '''
  def __init__(self, filename, dialect='excel-tab'):
...
def save_section(swriter, heading, rows):
  ...
  @param data: A tuple of section head and the section to save
  @type data: tuple
  @param writer: A csv writer obtained from the SectionWriter
...
def read_sections(data):
  ...
  '''
  grouper = GroupKey()
  for heading,row in groupby(data, grouper):
    row.next()
    yield heading,row


def _test():
  import doctest,sections
  return doctest.testmod(sections)
