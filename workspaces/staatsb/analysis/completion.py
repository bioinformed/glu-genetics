...
def completion(genos, droppedrows=0, droppedcols=0):
  ...
  colcomps = [None]+[ colcomp.setdefault(colkey,[0,0]) for colkey in islice(header,1,None) ]

  for row in genos:
    items = izip(colcomps,row)
...
def filter_columns(rows, limit):
  limit += 1
  for row in rows:
    yield list(islice(row,limit))
...
def option_parser():
  ...
  parser.add_option('-f', '--format', dest='format', metavar='NAME', type='string',
...
def main():
  ...
  genos = csv.reader(autofile(args[0]), dialect='excel-tab')

  if options.rowlimit:
    genos = islice(genos, options.rowlimit)

  if options.collimit:
    genos = filter_columns(genos, options.collimit)
