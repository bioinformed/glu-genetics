import csv
import sys

from biozilla.fileutils    import autofile,load_map
from itertools             import count

DATA_HEADER_ILLUMINA = ['IlmnID','Name','IlmnStrand','SNP','AddressA_ID','AlleleA_ProbeSeq','AddressB_ID',
                        'AlleleB_ProbeSeq','Chr','MapInfo','Ploidy','Species','CustomerStrand','IlmnStrand',
                        'IllumicodeSeq','TopGenomicSeq']

DATA_HEADER_AFFY = ['Probe Set ID','Affy SNP ID','dbSNP RS ID','Chromosome','Genome Version','DB SNP Version',
                    'Physical Position','Strand','ChrX pseudo-autosomal region','Cytoband','Flank','Allele A',
                    'Allele B','Associated Gene','Genetic Map','Microsatellite','Fragment Length Start Stop',
                    'Freq Asian','Freq AfAm','Freq Cauc','Het Asian','Het AfAm','Het Cauc','Num chrm Asian',
                    'Num chrm AfAm','Num chrm Cauc']

ASSAYOUT_HEADER  = ['ASSAY_ID','DESIGN_ALLELES','DESIGN_CODES','DESIGN_SCORE','DESIGN_SEQUENCE',
                    'DESIGN_STRAND','SNP_PANEL_ID','SNPANNO_ID','STATUS','TECHNOLOGY','VENDOR_ASSAY_ID']

COMPLEMENT_ALLELE = {'A':'T', 'T':'A', 'G':'C', 'C':'G', 'Y':'R', 'R':'Y', 'S':'S', 'W':'W','K':'M','M':'K',
                     'B':'V', 'V':'B', 'D':'H', 'H':'D','N':'N', '[':']', ']':'[', '/':'/'}

PANEL = ['100','200','300','400','400']

def reverse_complement(seq):
  rcseq = ''
  for a in seq:
    rcseq = COMPLEMENT_ALLELE[a] + rcseq
  return rcseq


def snp_assay_ilm(manifestfile,assayid,snpmap,assaymap,panelid):
  manifestfile = csv.reader(autofile(manifestfile))
  for row in manifestfile:
    if 'IlmnID' in row:
      header = row
      break
  assert header == DATA_HEADER_ILLUMINA

  for row in manifestfile:
    if '[Controls]' in row:
      raise StopIteration

    snpid            = row[1]
    dbsnp_build      = '124'
    genome_build     = '35'
    snp_unique_str   = snpid + '_' + dbsnp_build + '_' + genome_build

    snpannoid        = snpmap[snp_unique_str]
    snppanelid       = panelid
    assay_unique_str = snpannoid + '_' + snppanelid

    if assay_unique_str in assaymap:
      assay_id = assaymap[assay_unique_str]
    else:
      assay_id = assayid.next()
      assaymap[assay_unique_str] = assay_id

      designalleles = row[3].strip('[').strip(']')
      designcodes   = None
      designscore   = 1
      designstrand  = row[13]
      if designstrand == 'Top':
        designseq = row[15].upper()
      else:
        designseq = reverse_complement(row[15].upper())
      status        = None
      technology    = 'Infinium II'
      vendorassayid = row[0]
      yield [assay_id,designalleles,designcodes,designscore,designseq,designstrand,snppanelid,snpannoid,
             status,technology,vendorassayid]



def snp_assay_aff(manifestfile,assayid,snpmap,assaymap,panelid):

  def _check_id(ids):
    '''
    Create a mapping file between dbsnp ids and snpannoids;
    if unknown such as '---' used by Affy, use other unique ids for identification purpose only;
    If others, raise error.
    '''
    snpid = ids[1]
    if snpid[:2].upper() in ('RS','MI'):
      rowid        = snpid
    elif snpid == '---':
      rowid        = ids[0]
    else:
      raise ValueError, "Invalid dbsnp id in '%s'" % snpid
    return rowid

  manifestfile = csv.reader(autofile(manifestfile))
  for row in manifestfile:
    if 'Probe Set ID' in row:
      header = row
      break
  assert header == DATA_HEADER_AFFY

  for row in manifestfile:
    snpid            = row[2]
    rowid            = _check_id(row[1:3])

    dbsnp_build      = row[5].split(',')[0]
    genome_build     = '35'
    snp_unique_str   = rowid + '_' + dbsnp_build + '_' + genome_build

    snpannoid        = snpmap[snp_unique_str]
    snppanelid       = panelid
    assay_unique_str = snpannoid + '_' + snppanelid

    if assay_unique_str in assaymap:
      assay_id = assaymap[assay_unique_str]
    else:
      assay_id = assayid.next()
      assaymap[assay_unique_str] = assay_id
      designalleles = '/'.join(row[11:13])
      designcodes   = None
      designscore   = 1
      designstrand  = None
      designseq     = None
      snppanelid    = 400
      status        = None
      technology    = 'GeneChip'
      vendorassayid = row[1]
      yield [assay_id,designalleles,designcodes,designscore,designseq,designstrand,snppanelid,snpannoid,
             status,technology,vendorassayid]


def option_parser():
  import optparse

  usage = 'usage: %prog [options] file'
  parser = optparse.OptionParser(usage=usage)

  parser.add_option('-o', '--outfile',   dest='outfile',   metavar='FILE',   default= '-',
                    help='Output file for table snp_dim')
  parser.add_option('-M', '--assaymap',  dest='assaymap',  metavar='FILE',   default= '-',
                    help='Output file mapping dbsnp ids')
  parser.add_option('-m', '--snpmap',    dest='snpmap',    metavar='FILE',   default= '-',
                    help='Input file mapping dbsnp ids')
  parser.add_option('-c', '--inicount',  dest='inicount',  type = 'int')

  return parser



def main():
  parser = option_parser()
  options,args = parser.parse_args()
  snpmap = load_map(options.snpmap)

  assayoutfile = csv.writer(autofile(options.outfile,'w'))
  assayoutfile.writerow(ASSAYOUT_HEADER)
  assaymapoutfile  = csv.writer(autofile(options.assaymap,'w'))

  assayid = count(options.inicount)
  assaymap = {}

  datatype = 1
  for arg in args:
    if arg == 'ilm':
      datatype = 1
    elif arg == 'aff':
      datatype = 0
    else:
      panelid = PANEL.pop(0)
      if datatype:
        for assay in snp_assay_ilm(arg,assayid,snpmap,assaymap,panelid):
          if assay:
            assayoutfile.writerow(assay)
      else:
        for assay in snp_assay_aff(arg,assayid,snpmap,assaymap,panelid):
          if assay:
            assayoutfile.writerow(assay)

  for key,val in assaymap.iteritems():
    assaymapoutfile.writerow([key,val])



if __name__=='__main__':
  main()

