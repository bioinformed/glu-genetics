...
from   biozilla.genodata      import load_map, load_genomatrix_by_sample, rename_genomatrix_row
...
def main():
  ...
  samples = iter(load_genomatrix_by_sample(args[0], 'sdat', limit=options.locuslimit or None))

  if options.samplelimit:
    samples = islice(samples, options.samplelimit+1)

  if options.samplemap:
    samples = rename_genomatrix_row(samples, samplemap)
...
''' % (threshold*100,options.mincount))
  ...
  if options.tabularoutput:
    ...
    save_metadata_section(sw, analysis='dupcheck', analysis_version=__version__)
