...
from   biozilla.utils     import autofile, peekfirst
...
def load_genotypes(filename, format, locusset, sampleset, limit):
  rows = load_genomatrix_by_locus(filename,format,limit=limit)
  return rows
...
def main():
  ...
  test      = load_genotypes(args[0], options.testformat, None, None, None)
  reference = load_genotypes(args[1], options.refformat,  None, None, None)
