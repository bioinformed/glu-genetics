...
Abstract:      Biozilla genotype data management objects

Requires:      Python 2.5

Revision:      $Id: genodata.py 526 2007-03-20 15:14:40Z jacobske $
'''
...
from   itertools   import izip,islice,ifilter,imap,dropwhile,chain,groupby,repeat

from   utils       import pick,tally,any,all,peekfirst,unique
from   fileutils   import autofile,namefile,load_list,load_map,guess_format
...
#       strings are required.  However, it may move to biozilla.utils.
def intern_list(seq):
  '''
...
class GenotripleStream(GenotypeStream):
  ...
  def __init__(self, triples, samples=None, loci=None, order=None, unique=False, genorepr=snp_acgt, materialized=False):
    ...
    @type        unique: bool
    @param     genorepr: object represending the input/output encoding and
  ...
  def transformed(self, transform=None, mergefunc=None, **kwargs):
    ...
    if transform.genorepr is not None:
      ...
      if genorepr is not self.genorepr:
        triples = recode_genotriples(triples, self.genorepr, genorepr)
...
class GenomatrixStream(GenotypeStream):
  ...
  def transformed(self, transform=None, mergefunc=None, **kwargs):
    ...
    # Apply 4 stage transformation process:
    #
    ...
    if rowtransform.exclude is not None:
    ...
    if coltransform.exclude is not None:
    ...
    # Apply renamings
    if rowtransform.rename is not None:
    ...
    if transform.genorepr is not None:
      ...
      if genorepr is not self.genorepr:
        genos = recode_genomatrix(genos, self.genorepr, genorepr)
        packed = True
...
def load_genomatrixstream(filename, format=None, limit=None, genorepr=snp_acgt, unique=True):
  ...
  if format == 'hapmap':
    genos = load_hapmap_genotypes(filename,limit=limit,genorepr=genorepr.pack_strs)
    format = 'ldat'
  elif format == 'ldat':
    _,genos = load_genomatrix(filename,format=format,limit=limit,genorepr=genorepr.pack_strs,unique=unique)
  elif format == 'sdat':
    _,genos = load_genomatrix(filename,format=format,limit=limit,genorepr=genorepr.pack_strs,unique=unique)
  elif not format:
  ...
  return GenomatrixStream(genos, format, unique=unique, genorepr=genorepr, packed=True)
...
def save_genostream(filename, genos, format=None, mergefunc=None):
  ...
  if format == 'ldat':
    save_genomatrix(filename, genos.as_ldat(mergefunc), format, genorepr=genos.genorepr.strs_from_reps)
  elif format == 'sdat':
    save_genomatrix(filename, genos.as_sdat(mergefunc), format, genorepr=genos.genorepr.strs_from_reps)
  elif format in ('trip','genotriple'):
...
class GenoTransform(object):
  ...
  def __init__(self, include_samples, exclude_samples, rename_samples, order_samples,
                     ...
                     filter_missing=False, genorepr=None, repack=False):
    '''
  ...
  def from_options(options):
    ...
    return GenoTransform(options.includesamples, options.excludesamples, options.renamesamples, options.ordersamples,
                         ...
                         filter_missing=options.filtermissing, genorepr=options.outgenorepr)
...
class GenoSubTransform(object):
  ...
  def __init__(self, include, exclude, rename, order):
    ...
    '''
    if not isinstance(include, self.list_type):
      include = set(load_list(include))

    if not isinstance(exclude, self.list_type):
      exclude = set(load_list(exclude))

    if not isinstance(rename, self.map_type):
      rename  = load_map(rename)

    if not isinstance(order, self.list_type):
...
def merge_genomatrix_columns(genos, mergefunc):
  ...
  biozilla.genomerge.mergefunc_transpose_adapter.
...
def recode_genomatrix(genos, old_genorepr, new_repr):
  '''
  ...
  yield genos.next()
  for label,row in genos:
    row = new_repr.pack_genos(old_genorepr.genos_from_reps(row))
    yield label,row


def recode_genotriples(triples, old_genorepr, new_genorepr):
  '''
...
def load_hapmap_genotypes(file_or_name,limit=None,genorepr=snp_acgt.pack_strs):
  '''
  ...
  for line in gfile:
    ...
    assert len(genos) == n
    yield intern(fields[0]),genos


def load_genomatrix(file_or_name,format=None,limit=None,genorepr=snp_acgt.pack_strs,unique=True):
  '''
  ...
  @param file_or_name: a file name or file object
  @type  file_or_name: str or file object
  ...
  '''
  format,rows = _load_genomatrix(file_or_name,format=format,limit=limit,genorepr=genorepr)

  if unique:
    rows = unique_check_genomatrix(rows)

  return format,rows


def _load_genomatrix(file_or_name,format,limit,genorepr):
  '''See load_genomatrix'''
  ...
  try:
    ...
    raise ValueError, 'Input file "%s" is empty' % namefile(file_or_name)
  ...
  if format is not None and format_found not in ('',format):
    raise ValueError, 'Input file "%s" does not appear to be in %s format.  Found %s.' \
                        % (namefile(file_or_name),format,format_found)
...
def save_genomatrix(file_or_name,matrix,format=None,genorepr=snp_acgt.strs_from_reps):
  '''
  ...
  @type      genorepr: unary function
  '''
  gfile = autofile(file_or_name,'w')
  f = csv.writer(gfile,dialect='tsv')
  matrix = iter(matrix)
  header = matrix.next()
  n = len(header)
  f.writerow( list(chain([format or ''],header)) )
  for rowkey,data in matrix:
    assert len(data) == n
    f.writerow(list(chain([rowkey],genorepr(data))))


def load_genomatrix_by_locus(file_or_name,format,limit=None,genorepr=snp_acgt.pack_strs,unique=True):
  '''
  Load the genotype data in a locus major matrix format

  @param file_or_name: file name or file object
  @type  file_or_name: str or file object
  @param       format: the format of the genotype matrix file,
                 currently supports 'hapmap' or 'ldat'
  @type        format: str
  @param        limit: limit the number of columms loaded
  @type         limit: int or None
  @param     genorepr: function to convert list genotype strings to desired
  ...
  @param       unique: verify that rows and columns are uniquely labeled
                       (default is True)
  @type        unique: bool
  @return: sequence of locus names names followed by tuples of sample name and genotypes
  @rtype:  generator
  '''
  if format == 'hapmap':
    rows = load_hapmap_genotypes(file_or_name,limit=limit,genorepr=genorepr)
  elif format == 'ldat':
    _,rows = load_genomatrix(file_or_name,format=format,limit=limit,genorepr=genorepr,unique=unique)
  else:
    raise NotImplementedError,'File format is not supported'
    #FIXME: add to support other formats
  return rows


def load_genomatrix_by_sample(file_or_name,format,limit=None,genorepr=snp_acgt.pack_strs,unique=True):
  '''
  Load the genotype data in a sample major matrix format
...
def load_genotriples(file_or_name,limit=None,genorepr=snp_acgt.rep_from_str):
  '''
  ...
  @rtype:  generator
  '''
  rows = csv.reader(autofile(file_or_name),dialect='tsv')

  if limit:
    rows = islice(rows,limit+1)

  for sample,locus,geno in rows:
    yield intern(sample),intern(locus),genorepr(geno)


def save_genotriples(file_or_name,triples,genorepr=snp_acgt.str_from_rep):
  '''
  ...
  @type      genorepr: unary function
  '''
  out = csv.writer(autofile(file_or_name, 'w'),dialect='tsv')

  for sample,locus,geno in triples:
    out.writerow( [ sample,locus,genorepr(geno) ] )


def load_genotypes_as_genotriples(file_or_name,format,limit=None,genorepr=snp_acgt,unique=True):
  '''
  Load the genotype data as genotype triples from ldat,sdat,hapmap,genotriple file

  @param file_or_name: file name or file object
  @type  file_or_name: str or file object
  @param       format: the format of the genotype matrix file,
                       currently supports 'hapmap' or 'ldat' or 'sdat' or 'genotriple' or 'trip'
  @type        format: str
  @param        limit: limit the number of columms loaded
  @type         limit: int or None
  @param     genorepr: function to convert list genotype strings to desired
  ...
  >>> from StringIO import StringIO
  >>> t=load_genotypes_as_genotriples(StringIO("key\\t1420_11\\t1420_12\\nrs2070\\tAA\\tAG\\nrs3373\\tTT\\tTT\\n"),"ldat")
  >>> for s,l,g in t:
  ...   print s,l,g
  1420_11 rs2070 17
  1420_12 rs2070 19
  ...
  if format in ('ldat','hapmap'):
    rows = load_genomatrix_by_locus(file_or_name,format,limit,genorepr.pack_strs)
    genotriples = build_genotriples_by_locus(rows)
  elif format=='sdat':
    rows = load_genomatrix_by_sample(file_or_name,format,limit,genorepr.pack_strs)
    genotriples = build_genotriples_by_sample(rows)
  elif format in ('genotriple','trip'):
    genotriples = load_genotriples(file_or_name,limit,genorepr.rep_from_str)
  else:
    raise NotImplementedError,'File format is not supported'
    #FIXME: add to support other formats
  return genotriples
