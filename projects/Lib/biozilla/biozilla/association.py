...
from   numpy          import array,matrix,asarray,asanyarray,zeros,zeros_like, \
                             exp,nan,abs,arange,median
from   scipy          import stats

from   fileutils      import autofile,namefile
...
def load_phenos(filename,deptype=int,allowdups=False,verbose=2,errs=sys.stderr):
  ...
  if dups:
    errs.write('[ERROR] Duplicateheaders detected (n=%d)' % len(dups))
    if verbose > 1:
  ...
  def _phenos():
    ...
    note_msg = '[NOTICE] Phenotypes read from %s: ' \
    ...
    if verbose:
      errs.write(note_msg % (namefile(filename),len(header)-2,i,i-dropped,dropped,len(subjects)))
...
# FIXME: Needs docs+tests
class ModelLocus(object):
  def __init__(self, genos, geno_indices):
    self.genos        = genos
...
class LocusModelBuilder(object):
  def __init__(self,locus_header,pheno_header,phenos,genorepr,minmaf=0.01,mingenos=10):
  ...
  def build_model(self,term,loci):
    ...
    for lname in set(term.loci()):
      ...
      lmodel = ModelLocus(genos,self.geno_indices)
