...
def mapper2(astr,sep=None,join='1:'):
  if sep:
    p   = re.compile(sep)
    j1  = re.compile(':')
    j2  = re.compile(',')
    al  = p.split(astr)
    pars= j1.split(join)
    return pars[1].join(al[int(i)] for i in j2.split(pars[0]))
...
def mapper(astr,sep=None,join='1:'):
  if sep:
    p   = re.compile(sep)
    j1  = re.compile(':')
    j2  = re.compile(',')
    al  = p.split(astr)
    pars= j1.split(join)
    return pars[1].join(al[int(i)-1] for i in j2.split(pars[0]))
